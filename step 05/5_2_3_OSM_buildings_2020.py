# ---------------------------------------- File 5_1_1_OSM_buildings_2020 ---------------------------------------- #
# This piece of code imports all modules and libraries that are required for obtaining the buildings surface per neighborhood for 2020 via Open Street Map
import os
import time
import overpy
...
def parallelize(data, func, num_of_processes=10):
...
def parallelize_on_rows(data, func, num_of_processes=10):
    return parallelize(data, partial(run_on_subset, func), num_of_processes)

#___ Funtions for obtaining buildings areas from the OSM server, clipping it to neighborhoods, and obtaining the total area_______________________________________________________________________________________________________
...
def downloadXML(BB_Ext, loc_xml):
    api = overpass.API(timeout=120, endpoint=endpoint_overpassAPI, date="2020-12-31T23:59:59Z")
    request = f"""
        (
        way["building"="apartments"]({BB_Ext});
        way["building"="basilica"]({BB_Ext});
        way["building"="cathedral"]({BB_Ext});
        way["building"="chapel"]({BB_Ext});
        way["building"="church"]({BB_Ext});
        way["building"="commercial"]({BB_Ext});
        way["building"="entrance"]({BB_Ext});
        way["building"="farm"]({BB_Ext});
        way["building"="garage"]({BB_Ext});
        way["building"="hangar"]({BB_Ext});
        way["building"="hotel"]({BB_Ext});
        way["building"="house"]({BB_Ext});
        way["building"="industrial"]({BB_Ext});
        way["building"="mosque"]({BB_Ext});
        way["building"="retail"]({BB_Ext});
        way["building"="synagogue"]({BB_Ext});
        way["building"="temple"]({BB_Ext});
        way["building"="transformer_tower"]({BB_Ext});
        way["building"="wayside_shrine"]({BB_Ext});
        way["building"="yes"]({BB_Ext});
        );
        (._;>;);
        out ids geom;
        """
...
# It dissolves the multipolygons to create a single polygon for each area of buildings, and then explodes the dissolved polygons to create a separate row for each polygon.
...
def xml2gdf(loc_xml, input):
    ...
    if input['out_type'] == 'gpkg':
        rel_path = 'OSM_buildings.gpkg'
        loc_OSM = os.path.join(temp_dir, rel_path)
        gdf.to_file(loc_OSM, layer='OSM_buildings', driver="GPKG", OVERWRITE="YES")
...
def downloadOSM(input):
    ...
    # Clip the buildings areas based on the neighborhood's geometry
    gdf = clip(gdf, poly)
    return gdf

# This piece of code is used to calculate the total area of buildings for each neighborhood in the dataset which contains the geometries and information of all CBS 2020 neighborhoods.
# The sum_clipped_area() function takes a row from a GeoDataFrame, downloads OpenStreetMap data for the area defined by the 'geometry' field of the row, and calculates the total area of buildings in that area.
...
# The total area of buildings is calculated by summing the 'area' field of the returned GeoDataFrame, and this value is stored in the 'area_buildings' field of the row.
...
# The resulting GeoDataFrame, which now includes the total area of buildings for each neighborhood, is saved to a new GeoPackage file. Duplicate areas are dealth with. 
...
def sum_clipped_area(row, temp_dir, out_path): 
    ...
    row['area_buildings'] = merged_geometry.area
...
if __name__ == '__main__':
    ...
    rel_path = 'intermediate/TEMP_buildings_2020'
    temp_dir = os.path.join(script_dir, rel_path)

    rel_path = 'buildings.gpkg'
    out_path = os.path.join(temp_dir, rel_path)

    source_neighborhoods = 'output/5_1_3_OSM_water_2020.gpkg'
    ...
    source_neighborhoods_read.to_file("output/5_2_3_OSM_buildings_2020.gpkg", driver="GPKG")
