# ---------------------------------------- File 5_1_1_OSM_water_2020 ---------------------------------------- #
# This piece of code imports all modules and libraries that are required for obtaining the water surface per neighborhood for 2020 via Open Street Map
import os
import time
import overpy
...
#___ Funtions for obtaining water areas from the OSM server, clipping it to neighborhoods, and obtaining the total area_______________________________________________________________________________________________________
...
def getPolyFeatures(data):
    ...
    for feature in features:
        ...
        watertype = getType(json)
        data_list.append([osm_id, other_tags, shapely_geo, watertype])
...
# It dissolves the multipolygons to create a single polygon for each area of water, and then explodes the dissolved polygons to create a separate row for each polygon.
...
def xml2gdf(loc_xml, input):
    ...
    if input['out_type'] == 'gpkg':
        rel_path = 'OSM_water.gpkg'
        loc_OSM = os.path.join(temp_dir, rel_path)
        gdf.to_file(loc_OSM, layer='OSM_water', driver="GPKG", OVERWRITE="YES")
...
def downloadOSM(input):
    ...
    # Clip the water areas based on the neighborhood's geometry
    gdf = clip(gdf, poly)
    return gdf

# This piece of code is used to calculate the total area of water for each neighborhood in the dataset which contains the geometries and information of all CBS 2020 neighborhoods.
# The sum_clipped_area() function takes a row from a GeoDataFrame, downloads OpenStreetMap data for the area defined by the 'geometry' field of the row, and calculates the total area of water in that area.
...
# The total area of water is calculated by summing the 'area' field of the returned GeoDataFrame, and this value is stored in the 'area_water' field of the row.
...
# The resulting GeoDataFrame, which now includes the total area of water for each neighborhood, is saved to a new GeoPackage file. Duplicate areas are dealth with. 
...
def sum_clipped_area(row, temp_dir, out_path): 
    ...
    row['area_water'] = merged_geometry.area
...
if __name__ == '__main__':
    ...
    rel_path = 'intermediate/TEMP_water_2020'
    temp_dir = os.path.join(script_dir, rel_path)

    rel_path = 'water.gpkg'
    out_path = os.path.join(temp_dir, rel_path)

    source_neighborhoods = 'output/5_0_3_OSM_greenery_2020.gpkg'
    ...
    source_neighborhoods_read.to_file("output/5_1_3_OSM_water_2020.gpkg", driver="GPKG")
