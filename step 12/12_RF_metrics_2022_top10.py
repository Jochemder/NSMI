...
def load_rf_data(script_dir):
    ...
    X_train = pickle.load(open(os.path.join(script_dir, 'output/RF_2022_finalrun_top10/X_train.pkl'), 'rb'))
    X_test = pickle.load(open(os.path.join(script_dir, 'output/RF_2022_finalrun_top10/X_test.pkl'), 'rb'))
    y_train = pickle.load(open(os.path.join(script_dir, 'output/RF_2022_finalrun_top10/y_train.pkl'), 'rb'))
    y_test = pickle.load(open(os.path.join(script_dir, 'output/RF_2022_finalrun_top10/y_test.pkl'), 'rb'))
    rf = pickle.load(open(os.path.join(script_dir, 'output/RF_2022_finalrun_top10/random_forest_model.pkl'), 'rb'))
...
def generate_report(rf, X_test, y_test, CLASS_NAMES):
    ...
    metrics_df.to_excel('output/RF_2022_finalrun_top10/metrics/metrics_report.xlsx', index=False)
...
def generate_beeswarm_plot(shap_values_per_class, X_test_sample, rf, feature_names):
    ...
    for i, class_name in enumerate(rf.classes_):
        ...
        plt.tight_layout()
        plt.savefig(f'output/RF_2022_finalrun_top10/metrics/shap_beeswarm_plot_{class_name_verbose}.png', dpi=300)
        plt.close()

def retrieve_feature_names():
    with open("output/RF_2022_finalrun_top10/feature_index_mapping.pkl", "rb") as f:
...
def generate_shap_report(shap_values_per_class, rf, feature_names):
    ...
    shap_report_df.to_excel('output/RF_2022_finalrun_top10/metrics/shap_report_2022.xlsx')
    print('SHAP report saved to output/RF_2022_finalrun_top10/metrics/shap_report_2022.xlsx')
...
def save_top_features(shap_values_per_class, feature_names, top_n):
    ...
    output_dir = 'output/RF_2022_finalrun_top10/metrics'
    os.makedirs(output_dir, exist_ok=True)

    # Save the names of the top features to a text file
    output_file_path = os.path.join(output_dir, f'top_{top_n}_features_2022.txt')
...
def calculate_feature_importances(rf, X_test, y_test, feature_names, output_file_FI='output/RF_2022_finalrun_top10/metrics/feature_importance_report_2022.xlsx'):
    ...
    for i, class_name in enumerate(CLASS_NAMES):
        ...
        for feature_index in top_features_indices:
            ...
            try:
                ...
                plt.tight_layout()
                output_dir = f'output/RF_2022_finalrun_top10/metrics/pdp_{class_name}_{safe_feature_name}'
...
def plot_roc_curves(rf, X_test, y_test, CLASS_NAMES):
    ...
    plt.savefig('output/RF_2022_finalrun_top10/metrics/roc_curves.png')
    plt.close()
    
    # Save AUC values to an .xlsx file
    auc_df = pd.DataFrame.from_dict(roc_auc, orient='index', columns=['AUC'])
    auc_df.index = CLASS_NAMES
    os.makedirs('output/RF_2022_finalrun_top10/metrics', exist_ok=True)
    auc_df.to_excel('output/RF_2022_finalrun_top10/metrics/auc_values.xlsx')
    
    print('ROC curves and AUC calculated and plotted successfully!')

def calculate_shap_interaction(class_index, class_name, X_train, y_train, explainer, interaction_file):
    print(f'Generating SHAP interaction report for class: {class_name}')
    
    # Filter data for the current class
    class_mask = (y_train == class_index + 1).values.flatten()
    X_class = X_train[class_mask]

    # Calculate SHAP interaction values
    shap_interaction_values = explainer.shap_interaction_values(X_class)

    # Generate Excel report
    interaction_df = pd.DataFrame(shap_interaction_values[class_index], columns=X_train.columns, index=X_train.columns)
    interaction_df.to_excel(os.path.join(interaction_file, f'SHAP_interaction_report_{class_name}.xlsx'))

    print(f'SHAP interaction report for class {class_name} saved successfully!')

def generate_shap_interaction_report(X_train, y_train, rf, interaction_file):
    ...
    with mp.Pool(processes=mp.cpu_count()) as pool:
        ...
        args = [(class_index, class_name, X_train, y_train, explainer, interaction_file) for class_index, class_name in enumerate(CLASS_NAMES)]
        
        # Execute the worker function in parallel
        pool.starmap(calculate_shap_interaction, args)
...
if __name__ == '__main__':
    ...
    # Set file paths
    shap_values_file = os.path.join(script_dir, 'output/RF_2022_finalrun_top10/metrics/shap_values.pkl')
    output_file_FI = os.path.join(script_dir, 'output/RF_2022_finalrun_top10/metrics/feature_importance_report_2022.xlsx')
    interaction_file = os.path.join(script_dir, 'output/RF_2022_finalrun_top10/metrics')
    ...
    generate_shap_interaction_report(X_train, y_train, rf, interaction_file)