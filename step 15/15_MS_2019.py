...
model_path = 'output/RF_2019_finalrun_top10/random_forest_model.pkl'
...
def process_pickle_file(args):
    ...
    try:
        ...
        gdf = gpd.read_file("output/7_OMD_pt_2019.gpkg")
...
def process_all_files(pickle_dir, distance_range):
    ...
    df_original = pd.read_pickle("output/13_prepare_synthetic_population_2019.pkl")
...
if __name__ == "__main__":
    ...
    short_df = process_all_files('output/synthetic_population_2019/', short_distance_range)

    # Process medium distance files
    medium_df = process_all_files('output/synthetic_population_2019/', medium_distance_range)

    # Process long distance files
    long_df = process_all_files('output/synthetic_population_2019/', long_distance_range)

    # Save the aggregated results to pickle files
    short_df.to_pickle('output/MS_2019/MS_prediction_short.pkl')
    medium_df.to_pickle('output/MS_2019/MS_prediction_medium.pkl')
    long_df.to_pickle('output/MS_2019/MS_prediction_long.pkl')

    print("Aggregated results saved to output/MS_2019/")