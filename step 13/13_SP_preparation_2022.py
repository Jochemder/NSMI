# ------------------------------------------- file 13_SP_preparation_2022.py -------------------------------------------
...
file_path = 'output/1_ODIN_2022.csv'

# Read the CSV file into a pandas DataFrame with the correct parameters
target_percentage_2022 = pd.read_csv(file_path, encoding='ISO-8859-1', sep=';')

# Check if 'Age' column exists
if 'Age' not in target_percentage_2022.columns:
    raise KeyError("The 'Age' column is missing from the DataFrame")

# Split the DataFrame into chunks
num_chunks = 45
chunks = np.array_split(target_percentage_2022, num_chunks)
...
target_percentage_2022 = pd.concat(age_results)
...
# Concatenate the results back into a single DataFrame
target_percentage_2022 = pd.concat(household_results)

# Save the changed dataset to a new pickle file
output_path = 'output/13_prepare_synthetic_population_2022.pkl'
target_percentage_2022.to_pickle(output_path)

# Display the first few rows to verify the new columns
print(target_percentage_2022.head())